...
const DetailsCard = (props: Props) => {
  ...
  return (
    <ScrollView>
      <Text style={{ color: 'black', marginVertical:10,marginHorizontal:10,marginTop:50}} variant='headlineMedium'>{props.title}</Text>
      <Card style={{ backgroundColor: theme.colors.background }} contentStyle={{ width: Dimensions.get("window").width }}>
        {
         ...
         <Card.Content>
          <Text
            ...
            style={{ textAlign: 'left', marginVertical: 10 }}>{props.content}</Text>
        </Card.Content> 
      </Card>
    </ScrollView>
  )
